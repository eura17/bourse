        ...
        if self.datetime.second == 1:
            for ticker in self.tickers:
                ...
                if len(candles) >= self.slow_period:
                    fast_sma = sum([c.close for c in candles[-self.fast_period:]]) / self.fast_period
                    slow_sma = sum([c.close for c in candles[-self.slow_period:]]) / self.slow_period
                    macd = fast_sma - slow_sma
                    if len(candles) >= self.n and \
                            len(self.macd) >= self.signal_period:
                        self.macd = self.macd[-self.signal_period:]
                        signal_sma = sum(self.macd[-self.signal_period:]) / self.signal_period
